  ...
  def spec = suite("TournamentRoutes form parsing")(
    ...
    test("errorToResponse for conflict") {
      for
        ...
        id = extractId(body)
        _ <- tournament.service.TournamentService.join(
          tournament.domain.model.TournamentId(id),
          tournament.domain.model.BotRef(tournament.domain.model.BotId("bot1"), "Bot1"))
