...
object JsonCodecsDecoderSpec extends ZIOSpecDefault:
  ...
  def spec = suite("JsonCodecs decoders")(
    ...
    test("Tournament JSON with winner") {
      val t = tournament.domain.tournament.Tournament(
        id = TournamentId("t1"),
        config = TournamentConfig("W", 1, tournament.domain.model.Clock(60, 0), true, TournamentFormat.Swiss, StartPosition.Standard, 1),
    ...
    test("Tournament JSON with FromFen start position") {
      val t = tournament.domain.tournament.Tournament(
        id = TournamentId("t1"),
        config = TournamentConfig("F", 1, tournament.domain.model.Clock(60, 0), true, TournamentFormat.Swiss, StartPosition.FromFen("custom fen"), 1),
    ...
    test("Game JSON with black winner and FromFen") {
      import tournament.domain.game.Game
      val g = Game(
