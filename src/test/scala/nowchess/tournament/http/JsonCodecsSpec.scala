...
import nowchess.tournament.http.codec.JsonCodecs
...
object JsonCodecsSpec extends ZIOSpecDefault:
  ...
  def spec = suite("JsonCodecs")(
    ...
    test("Clock round-trips"):
      val c = nowchess.tournament.domain.model.Clock(300, 5)
      val json = c.toJson
      assertTrue(json.fromJson[nowchess.tournament.domain.model.Clock] == Right(c))
    ,
    ...
    test("Tournament encodes"):
      val t = Tournament(
        id = TournamentId("t1"),
        config = TournamentConfig("Test", 2, nowchess.tournament.domain.model.Clock(300, 5), rated = true, TournamentFormat.Swiss, StartPosition.Standard, 1),
    ...
    test("TournamentConfig encodes"):
      val c = TournamentConfig("N", 2, nowchess.tournament.domain.model.Clock(300, 5), true, TournamentFormat.Swiss, StartPosition.Standard, 1)
